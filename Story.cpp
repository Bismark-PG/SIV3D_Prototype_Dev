...
void Story_Manager::StartStory(StoryID id)
{
	...
	case StoryID::Tutorial_Name:
		...
		m_currentScript = {
			...
			{m_playerStats.partnerName, U"(今から脳内に色々な情報を流し込むので頑張って耐えて下さい。)"},
			{m_playerStats.partnerName, U"(それでは参ります。)"},
	...
	case StoryID::Tutorial_Battle:
		...
		m_currentScript = {
			...
			{U"[name]", U"(分かっていた事ですがやはり勝率は)"},
			{U"[name]", U"(現在限りなくゼロに近いですね。)"},
			{U"[name]", U"(人間様、例の言葉を。)"},
			{m_scriptProtagonist, U"分かってたなら最初からで良かったんじゃないか...？"},
			{U"[name]", U"(私は子供のようなものなので、\n 自分がどの程度通用するのか試したかったのですよ、人間様。)"},
	...
	case StoryID::Tutorial_Introduce:
		...
		m_currentScript = {
			...
			{m_scriptProtagonist, U"さっき名付けたばっかりだけど、\n お前に 「[name]」って名前があるように、 \n僕にも名前があるんだよ。"},
			{U"[name]", U"(成程。では何と呼べば？)"},
	...
	case StoryID::Day1_End:
		...
		{U"[player]", U"『様』はいらないよ。\n というか脳に直接喋るの慣れないんだけど普通に話せない？"},
		{U"[name]", U"([player]がそう思えば) \n(口と発声器官が創られると思いますよ。)"},
		{U"[name]", U"(それと『普通に』はちょっと傷つきますよ。)"},
		{U"[name]", U"(私のこれまでの『普通』はこれなんですから。)"},
		{U"[name]", U"(今の時代は多様性ですよ。)"},
		{U"[name]", U"(都市伝説にも適応して下さい。)"},
		{U"[player]", U"なんかごめん。"},
		...
		{U"[player]", U"その情報に僕とお前で齟齬があったらいけないだろ、\n だからさっきのおさらいをさせてくれ。"},
		{U"[name]", U"成程、多すぎる情報による弊害その②ですね。御意に。"},
		// --- (アルティメット説明タイム) --- //
		{U"[player]", U"そもそもの話、[都市伝説はこの世に実在する] んだな？"},
		{U"[name]", U"はい。先程目にした口裂け女や私がその最たる証拠ですが、\n実在するかどうかで言うとグレーですね。"},
		{U"[name]", U"言うならば我々は『真になった嘘』ですので。"},
		{U"[player]", U"噂で出来た情報の集合体、か。"},
		{U"[name]", U"はい。我々は人などから出される噂によって形を成し、 \n力を手に入れます。"},
		{U"[name]", U"その都市伝説の中の１体に、私たちは監禁されています。"},
		{U"[name]", U"私はその都市伝説を倒そうと思い、貴方に協力を仰ぎました。"},
		{U"[player]", U"色々訊きたい事はあるけど...まずなんで僕の助けがいるんだ？"},
		{U"[name]", U"それは『何故私が助けを必要としているか』でしょうか。 \n『何故人間に助けを求めたか』でしょうか。"},
		{U"[name]", U"それとも、\n『何故協力者に [player] を選んだか』？"},
		{U"[player]", U"まぁ大体全部かな...先２つはざっくりわかるけど...。"},
		{U"[name]", U"それでは簡潔に。"},
		{U"[name]", U"①私が都市伝説として未熟だからです。\n都市伝説ではあるものの、まだ何者でもない、\n中途半端な個体なので。"},
		{U"[name]", U"②都市伝説を育てるには人間が最適だからです。 \n現代社会に於いては誰もが片手間で噂を取り入れ、 \nそして発信する事が出来るようになってしまったので。"},
		{U"[name]", U"③たまたま近くにいたあの情報に耐えうる人間が\n[player]だったので。"},
		{U"[player]", U"たまたまかよ...\nその情報に耐えうるってのは何を以て判断してるんだ？"},
		{U"[name]", U"与える情報の受け入れやすさですね。 \n[player]は、都市伝説がお好きで？"},
		{U"[player]", U"まぁ動画とかで結構見たりはするけど... \nそれって見て判断できるものなのか？それとも都市伝説の能力？"},
		{U"[name]", U"...？分かりませんよ、そんなことは。 \n[player]が耐えうる人間じゃなければ \n別な人間を探していました。"},
		{U"[player]", U"...もし情報に耐えられなかった場合はどうなるんだ？"},
		{U"[name]", U"廃人になるでしょうね。"},
		{U"[player]", U"お前...人間を使い捨ての駒みたいに...！"},
		{U"[name]", U"先程も申し上げましたが、 \n貴方(人間)の価値観を押し付けないで下さい。"},
		{U"[name]", U"この世界の解放の重大さに比べれば \nそんなことは些事でしかないですよ。"},
		{U"[player]", U"人の人生が些事であっていい訳がないだろ！"},
		{U"[name]", U"それにこの世界は解放された後恐らく \n攫われる前に時間に回帰しますよ。"},
		{U"[name]", U"それでも貴方からすれば憤慨するようなことなのでしょうか？"},
		{U"[player]", U"そういうことじゃあ..."},
		{U"[player]", U"いや、もういい。"},
		{U"[name]", U"理解していただけましたか。"},
		{U"[player]", U"お前達とは相容れないってことがね。"},
		{U"[player]", U"僕達の情報から出来たとはいえ、 \nお前達は情報の上辺だけを切り取って存在しているんだ。"},
		{U"[player]", U"だから決して理解することも無いし、されることも無いと思うよ。"},
		{U"[name]", U"左様ですか。 \n私としてもお友達ごっこをするつもりなど毛頭ありません。"},
		{U"[name]", U"そも回帰されれば無かったことになりますので、 \n元から７日のみの交流だと思っています。"},
		{U"[player]", U"そうか。"},
		{U"[player]", U"ところでその７日てのはどこから出てきた数字だ？"},
		{U"[name]", U"奴と戦う期限です。 \nそれ以降、奴は大成して勝算はほぼゼロに等しくなるでしょう。"},
		{U"[player]", U"１週間てのは大分短いが... \nその僕達を閉じ込めている都市伝説についての仔細情報が欲しい。"},
		{U"[name]", U"はい"},
		{U"[name]", U"その都市伝説の名は『きさらぎ』。 \nかつて『きさらぎ駅』という名で存在していた都市伝説です。"},
		{U"[name]", U"近代日本においてあまりにも名のある都市伝説でしたが、 \nそれは大きくなりすぎました。"},
		{U"[name]", U"他の類似点を持つ都市伝説の吸収を繰り返し、 \nあまつさえ都市そのものとなった、正しく『都市』伝説です。"},
		{U"[name]", U"それは人や動物、都市伝説を我が身の中に誘い、\n自分の分け身を這わせ、噂を流させることによって、 \n現在進行形で強くなり続けています。"},
		{U"[player]", U"なんて厄介な... \n場所系の都市伝説だしこれといって弱点も思いつかないけど...。"},
		{U"[player]", U"戦うってことは勝てる見込みはあるんだろ？"},
		{U"[name]", U"私と[player]の努力次第ですね。"},
		{U"[name]", U"私は今は未熟ではありますが、 \n裏を返せば何者にも成れることができます。"},
	...
	case StoryID::Day2_Start:
		...
		{U"[name]", U"『探索』では、私の噂を広める事で私の強化ができたり、 \n有効的なアイテムを入手することができるかもしれません。"},
		{U"[name]", U"『戦闘』では、他の都市伝説と戦闘をして勝つことで、 \n都市伝説達に私の噂が広まり強化に繋がります。"},
		{U"[name]", U"他の都市伝説の狂化の恐れがあるので \n丑三つ時前には帰りましょう。"},
	...
	case StoryID::FinalBattle_Start:
		...
		{U"[player]", U"...。"},
		{U"[player]", U"なぁ[name]、\nお前はどうしてここから出ようと思ったんだ？"},
		{U"[name]", U"...？なんですか、藪から棒に。"},
		{U"[player]", U"いや、最初の方に訊こうと思ってたんだけど \n訊くタイミング逃しちゃってさ。"},
		{U"[player]", U"今がゆっくり話せる最後の時間かもだろ？"},
		{U"[name]", U"そうですね...\n強いて言うのならここに居るべきではないと思ったからです。"},
		{U"[name]", U"朧げではありますが、 \n私を形成していた数少ない情報の中に、 \n場所にまつわるものがあったのだと思います。"},
		{U"[player]", U"花子さんがトイレに縛られるように、\n[name]もどこかの場所に縛られる \n都市伝説だったってことか。"},
		...
		{U"[player]", U"うん。記憶が消えたとしても、心に刻んだものは、\nそいつの在り方として変わることはないよ。\n僕も、[name]も。"},
	...
	case StoryID::Ending_Good:
		...
		{U"[name]", U"きさらぎを倒したことにより、 \nこの街の瓦解が始まったようですね。"},
		{U"[name]", U"もう暫くしたら、私達は攫われる前の状態に回帰するでしょうね。 \n場所も、記憶も。"},
		{U"[player]", U"[name]ともここでお別れか。"},
		{U"[player]", U"ただの協力関係だったけど、別れとなると流石に名残惜しいな。"},
		{U"[name]", U"...そうですね。"},
		{U"[name]", U"[player]。"},
		{U"[name]", U"もしかしたら今生の別れになるかもしれないので、 \n改めて貴方に感謝を。"},
		{U"[player]", U"そうだな、もう出会えるか分からないし...。"},
		{U"[name]", U"いえ、もし回帰したら私は消滅、 \nないしは死ぬ可能性がおおいにありますので。"},
		{U"[player]", U"っ...！？どういうことだよ、それ...！？"},
		{U"[name]", U"私は所詮都市伝説のなりそこないです。"},
		{U"[name]", U"今でこそ[player]に協力してもらい、 \nここまで育つことはできましたが、あの時点での私は、\n本当に運よく存在を保っていただけですので。"},
		...
		{U"[player]", U"『記憶が消えたとしても、\n心に刻んだものは、\n在り方として変わらない』"},
		{U"[player]", U"って。"},
		{U"[player]", U"だから、回帰したとしても、\n[name]はあの頃の[name]よりも"},
		{U"[player]", U"少し人間らしくなって、 \nしぶとく生きるって、 \n僕は信じてる。"},
		{U"[name]", U"[player]...。"},
